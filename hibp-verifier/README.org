...
#+begin_src sh
HIBP_DATA_DIR=/path/to/hibp-data cargo test -p hibp-verifier
#+end_src

Or using the default location:

#+begin_src sh
# Assumes dataset exists at hibp-rs/pwnedpasswords-bin/
...
through =FFFFF.bin=. Each file contains sorted 8-byte sha1t64 records for the
corresponding SHA1 prefix.

** Record Layout

Each record is the first 8 bytes of a SHA1 hash (truncated to 64 bits). Records are stored in ascending sorted order, enabling binary search.
...
#+begin_src rust
let offset = mid * RECORD_SIZE;  // RECORD_SIZE = 8
