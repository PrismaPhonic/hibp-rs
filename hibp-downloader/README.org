...
from [[https://github.com/oschonrock/hibp][oschonrock/hibp]] but we only store the 6 byte suffix. The first 2 bytes are redundant, as the file names themselves contain a 2.5 byte prefix.
